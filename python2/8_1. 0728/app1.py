...
font_path = 'python2/0728/NanumGothic.ttf'
...
def get_database():
    conn = sqlite3.connect('python2/0728/customer.db')
    return conn
...
def add():
    customer_id = request.form['customer_id'] # request.form은 post 방식일 때 딕셔너리처럼 사용 가능
    # customer_id = request.args['customer_id'] # request.args은 get 방식일 때 딕셔너리처럼 사용 가능
...
def statistics():
    ...
    for i, v in enumerate(sales):
        plt.text(i, v + 1000, f'{v:,}원', ha='center', va='bottom', fontsize=10, color='blue', fontweight='bold')
    plt.tight_layout()
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()

    plt.title('일자별 매출 현황', pad=20)
    plt.xlabel('구매 일자')
    plt.ylabel('매출액(원)')
    plt.xticks(rotation=0, fontsize=10)
    plt.grid(True)
    chart_path = "python2/0728/static/images/sales_chart.png"
    ...
    plt.pie(gender_sales, labels=gender_labels, autopct='%1.1f%%', colors=['lightblue', 'pink'])
    plt.title('성별 매출 분포')
    gender_chart_path = "python2/0728/static/images/gender_chart.png"
    ...
    plt.pie(age_sales, labels=age_labels, autopct='%1.1f%%', colors=colors)
    plt.title('연령대별 매출 분포')
    age_chart_path = "python2/0728/static/images/age_chart.png"
